...
def self_train(args, selftrain_dataset, model, tokenizer, labels, pad_token_label_id):
    logging.info('call self_train method')

    if args.local_rank in [-1, 0]:
        tb_writer = SummaryWriter()

    args.train_batch_size = args.per_gpu_train_batch_size * max(1, args.n_gpu)
    train_sampler = RandomSampler(selftrain_dataset) if args.local_rank == -1 else DistributedSampler(selftrain_dataset)
    train_dataloader = DataLoader(selftrain_dataset, sampler=train_sampler, batch_size=args.train_batch_size)
    ...
    logger.info("  Num examples = %d", len(selftrain_dataset))
    ...
    for t3 in range(int(self_training_steps/len(train_dataloader)) + 1):
        ...
        epoch_iterator = tqdm(train_dataloader, desc="SelfTrain, Iteration", disable=args.local_rank not in [-1, 0])

        for step, batch in enumerate(epoch_iterator):
            if global_step % args.self_training_update_period == 0:
                teacher_model = copy.deepcopy(model) #.to("cuda")
                teacher_model.eval()
                for p in teacher_model.parameters():
                    p.requires_grad = False
            model.train()
            batch = tuple(t.to(args.device) for t in batch)  # GPU or CPU     
            inputs = {
                        'input_ids': batch[0],
            ...
            loss = calc_loss(input = torch.log(softmax(logits)), \
